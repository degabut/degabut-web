...
import { A, Button, Container, Icon, Spinner, Text, useScreen } from "@common";
import { IS_DISCORD_EMBEDDED, SPOTIFY_CLIENT_ID } from "@constants";
import { useSettings } from "@settings";
import { SpotifyConnectionState, useSpotify } from "@spotify";
...
export const Spotify: Component = () => {
	...
	createEffect(() => {
		if (spotify.state() === SpotifyConnectionState.Empty) {
			spotify.initialize();
		}
	});
	...
	return (
		...
		>
			<Match when={spotify.state() === SpotifyConnectionState.Authenticating}>
				<Container size="full" extraClass="flex-col-center justify-center h-full">
					<Spinner size="3xl" />
				</Container>
			</Match>
			<Match when={spotify.state() === SpotifyConnectionState.Disconnected}>
