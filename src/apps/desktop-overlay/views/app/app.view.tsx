...
const ProvidedApp: Component = () => {
	...
	useBeforeLeave((e) => {
		if (!e.to.toString().startsWith("/desktop-overlay") && IS_DESKTOP) e.preventDefault();
	});
	...
	return (
		<Container size="full" padless extraClass="flex-col-center space-y-6 h-full bg-black/50">
			<div class="flex-row-center justify-center space-x-4 bg-black/90 w-full py-5">
				<img class="w-12 h-12" src="/android-chrome-512x512.png" />
				<Text.H1 class="text-3xl font-brand font-black tracking-wide">degabut</Text.H1>
			</div>
